...
%% 
 ...
 \documentclass[12pt, a2paper, landscape, margin=2mm, innermargin=4mm,
...
%  \tikzposterlatexaffectionproofon %shows small comment on how the poster was made at bottom of poster
 ...
 \author{Sandipan Mandal \& Teekam Chand Mandan\vspace{-1.0cm}}
 \title{Knowledge Distillation and its Variations\vspace{-1.5cm}}
  
% \settitle{\centering{\bfseries \fontsize{20}{40} \sc \@title \par}}
...
\addbibresource{cs698o_project.bib}
 ...
 \block[titleinnersep=2.5mm,bodyinnersep=4mm]{Knowledge Distillation using GANs}{Here we generalize the approach we have taken earlier. It is based on GANs. Though it uses GANs,
...
\column{0.34}
\block[titleinnersep=2mm,bodyinnersep=3mm]{Knowledge Distillation using Wasserstein GANs}{This aproach is exactly the same as the one above, except that here we use Wasserstein GANs (WGANs) instead of vanilla GANs. As a result the loss function is slightly different. The loss functions are:
$$L(W_D) = -\gamma[f_{real}(x) - f_{fake}(x)]$$ $$L(W_G) = \beta H(y_{true}, P_S) - \alpha f_{fake}(x) $$ where $f_{real}(x) = D(T(x))$ and $f_{fake}(x) = D(G(x))$}

\block[titleinnersep=4mm,bodyinnersep=4mm]{Knowledge Distillation (Schematic Diagram)}
{
    \begin{tikzfigure}
        \includegraphics[width=0.3\textwidth, height=0.18\textwidth]{images/KD.png}
    \end{tikzfigure}
}
...
{
    \begin{tikzfigure}
        \includegraphics[width=0.22\textwidth, height=0.1\textwidth]{images/Fitnet.png}
    \end{tikzfigure}
}

\block[titleinnersep=2mm,bodyinnersep=3mm]{Experiments (Architecture - I)}{
Exact architecture of \textbf{T} and \textbf{S} are different for different datasets but in each case both of them are fully connected networks with \textbf{S} having typically $\sim 50\%$ as much parameters compared to \textbf{T}.
\begin{tikzfigure}
%   \centering
...
\end{tikzfigure}
        ...
        %  \end{subcolumns}
         ...
         \block[titleinnersep=3mm,bodyinnersep=4mm]{References}{\printbibliography[heading=none]}
