...
model_name_save_student = 'mnist_paper_student'
data_dir = 'MNIST_data/'
...
with graph_teacher.as_default():
    ...
    layer1_weights_teacher = tf.Variable(tf.truncated_normal([image_size * image_size * num_channels, num_hidden], stddev=0.1), name='l1ws')
    layer1_biases_teacher = tf.Variable(tf.zeros([num_hidden]), name='l1bs')
    
    # Hidden1 to Hidden2 Layer
    layer2_weights_teacher = tf.Variable(tf.truncated_normal([num_hidden, num_hidden], stddev=0.1), name='l2ws')
    layer2_biases_teacher = tf.Variable(tf.constant(1.0, shape=[num_hidden]), name='l2bs')

    # Hidden2 to Output Layer
    layer3_weights_teacher = tf.Variable(tf.truncated_normal([num_hidden, num_labels], stddev=0.1), name='l3ws')
    layer3_biases_teacher = tf.Variable(tf.constant(1.0, shape=[num_labels]), name='l3bs')
    ...
    logits_train = student_model_train(tf_train_dataset)
    logits_eval = student_model_eval(tf_train_dataset)
