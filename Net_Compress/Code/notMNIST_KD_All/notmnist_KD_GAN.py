...
export_dir = 'notMNIST_Model_Student_KD_GAN/'
...
model_name_save_student = 'notmnist_student_KD_GAN'
...
def Train_Student(session):
  ...
  model_saver.save(session, export_dir + model_name_save_student + '_' + str(alpha) + '_' + str(beta) + '_' + str(gamma), write_meta_graph=True)
...
with graph_student_KD.as_default():
  ...
  pred_disc_student2 = tf.log(tf.maximum(1 - tf.nn.sigmoid(logits_disc_student), 1e-9))

  # Real
  logit_disc_teacher = discriminator_model(logits_teacher_eval)
  # pred_disc_teacher = tf.nn.softmax(logit_disc_teacher) # may be sigmoid
  pred_disc_teacher = tf.log(tf.maximum(tf.nn.sigmoid(logit_disc_teacher), 1e-9))

  loss_disc = -gamma*tf.reduce_mean(pred_disc_teacher + pred_disc_student2)
