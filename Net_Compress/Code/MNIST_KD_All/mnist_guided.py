...
with graph_student_guided.as_default():
  ...
  optimizer_student = tf.train.RMSPropOptimizer(learning_rate=0.00007).minimize(loss_student, var_list=student_first_half_params)
  ...
  with tf.device(current_device):
    ...
    with tf.Session(graph=graph_student_guided, config=config) as session:
      ...
      try:
        ...
        saver = tf.train.Saver(var_list=[layer_regressor_student])
        saver.save(session, export_dir + model_name_save_regressor, write_meta_graph=True)
