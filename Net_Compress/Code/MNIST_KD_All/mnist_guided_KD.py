...
model_name_save_student = 'mnist_student_guided_KD'
...
def Train_Student(session, flag=0):
...
with graph_student_guided.as_default():
  '''Input data'''
  ...
  def teacher_model_train(data):
    ...
    return reshape, tf.matmul(hidden, layer4_weights_teacher) + layer4_biases_teacher
  ...
  def student_model(data):
    ...
    # Readout Layer: Softmax Layer
    return reshape, tf.matmul(hidden, layer6_weights_student) + layer6_biases_student
  ...
  optimizer_student = tf.train.GradientDescentOptimizer(learning_rate=0.00007).minimize(loss_student, var_list=student_parameters)
  ...
  # optimizer_student2 = tf.train.GradientDescentOptimizer(learning_rate=0.00001).minimize(loss_student, var_list=student_parameters)
...
def train_student_KD():
  ...
  with tf.device(current_device):
    ...
    with tf.Session(graph=graph_student_guided, config=config) as session:
      ...
      # if flag == 2:
      model_saver = tf.train.Saver(var_list=student_parameters)
      model_saver.restore(session, export_dir + '0' + model_name_save_student + str(alpha) + '_' + str(T))
