...
with graph_student.as_default():
  ...
  # Conv3 to FC1 Layer
  final_image_size = 4
  layer4_weights_student = tf.Variable(tf.truncated_normal([final_image_size * final_image_size * depth, num_hidden], stddev=0.1), name='l4wt')
  layer4_biases_student = tf.Variable(tf.constant(1.0, shape=[num_hidden]), name='l4bt')

  # FC1 to FC2 Layer
  layer5_weights_student = tf.Variable(tf.truncated_normal([num_hidden, num_hidden], stddev=0.1), name='l5wt')
  layer5_biases_student = tf.Variable(tf.constant(1.0, shape=[num_hidden]), name='l5bt')

  # FC2 to FC3 Layer
  layer6_weights_student = tf.Variable(tf.truncated_normal([num_hidden, num_labels], stddev=0.1), name='l6wt')
  layer6_biases_student = tf.Variable(tf.constant(1.0, shape=[num_labels]), name='l6bt')
  ...
  optimizer_student = tf.train.GradientDescentOptimizer(learning_rate=0.000001).minimize(loss_student)
